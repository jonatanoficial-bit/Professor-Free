import { db, uid, getServerUrl } from "./db";
import type { SyncChange } from "../types/models";
import { api } from "./api";

export async function enqueueChange(change: Omit<SyncChange, "id">) {
  await db.syncQueue.put({ ...change, id: uid("chg") });
}

export async function markSynced(changeIds: string[]) {
...
async function applyUpsert(entity: SyncChange["entity"], payload: any) {
  switch (entity) {
    case "professor":
      await db.professor.put(payload);
      break;
    case "school":
    ...
    case "student":
      ...
      break;
    case "lessonLog":
      await db.lessonLogs.put(payload);
      break;
  }
}
...
async function applyDelete(entity: SyncChange["entity"], entityId: string) {
  switch (entity) {
    case "professor":
      await db.professor.delete(entityId);
      break;
    case "school":
      await db.schools.delete(entityId);
      break;
    case "class":
      await db.classes.delete(entityId);
      break;
    case "student":
      await db.students.delete(entityId);
      break;
    case "lessonLog":
      await db.lessonLogs.delete(entityId);
      break;
  }
}
